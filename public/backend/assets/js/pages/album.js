...
$(document).ready(function () {
    $(document).on('click', 'a[data-ajax-album-add-popup="true"]', function () {
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {
                    if (response.action == 'select') {
                        ...
                        $('.display-album-list-html').html(response.albumListData);
                        feather.replace();
                    }
                    form[0].reset();
    ...
    $(document).off('submit', '#albumEditForm').on('submit', '#albumEditForm', function (event) {
        event.preventDefault();
        var form = $(this);
        var submitButton = form.find('button[type="submit"]');
        $('.form-control').removeClass('is-invalid');
