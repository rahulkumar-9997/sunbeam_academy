...
$(document).ready(function () {
    $(document).on('click', 'a[data-ajax-disclosure-add-popup="true"]', function () {
    ...
    $(document).off('submit', '#disclosureAddForm').on('submit', '#disclosureAddForm', function (event) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {                    
                    $('.display-disclosure-list-html').html(response.disclosureListData);
                    feather.replace();
                    form[0].reset();
    ...
    $(document).on('click', 'a[data-ajax-edit-disclosure="true"]', function () {
        var title = $(this).data('title');
        var size = ($(this).data('size') == '') ? 'md' : $(this).data('size');
        var url = $(this).data('url');
        var data = {
    ...
    $(document).off('submit', '#disclosureEditForm').on('submit', '#disclosureEditForm', function (event) {
        event.preventDefault();
        var form = $(this);
        var submitButton = form.find('button[type="submit"]');
        $('.form-control').removeClass('is-invalid');
